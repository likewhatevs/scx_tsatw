...
int ubench_main(int argc, const char *const argv[])
{
	...
	for (index = 1; index < UBENCH_CAST(size_t, argc); index++) {
		...
		if (0 ==
		    ubench_strncmp(argv[index], help_str, strlen(help_str))) {
			...
			filter = argv[index] + strlen(filter_str);
					       ...
					       strlen(output_str))) {
			ubench_state.output = ubench_fopen(
				argv[index] + strlen(output_str), "w+");
		} else if (0 == ubench_strncmp(argv[index], list_str,
					       strlen(list_str))) {
			for (index = 0; index < ubench_state.benchmarks_length;
			     index++) {
				UBENCH_PRINTF(
					"%s\n",
	...
	for (index = 0; index < ubench_state.benchmarks_length; index++) {
		if (ubench_should_filter(filter,
					 ubench_state.benchmarks[index].name)) {
			continue;
		}
...
#undef UBENCH_MIN_ITERATIONS
		...
		for (mndex = 0; (mndex < 100) && (result != 0); mndex++) {
			...
			iterations = iterations *
				     (UBENCH_CAST(ubench_int64_t, mndex) + 1);
			...
			for (kndex = 0; kndex < iterations; kndex++) {
				const double v =
					UBENCH_CAST(double, ns[kndex] - avg_ns);
				deviation += v * v;
			}

			deviation = sqrt(deviation /
					 UBENCH_CAST(double, iterations));

			/* Confidence is the 99% confidence index - whose magic value is 2.576. */
			confidence = 2.576 * deviation /
				     sqrt(UBENCH_CAST(double, iterations));
			confidence =
				(confidence / UBENCH_CAST(double, avg_ns)) *
				100.0;

			deviation = (deviation / UBENCH_CAST(double, avg_ns)) *
				    100.0;
		...
		if (ubench_state.output) {
			fprintf(ubench_state.output,
				"%s, %" UBENCH_PRId64 ", %f, %f,\n",
				ubench_state.benchmarks[index].name,
				best_avg_ns, best_deviation, best_confidence);
		}

		{
			...
			printf("%s%s%s %s (mean ", colour, status,
			       colours[RESET],
			       ubench_state.benchmarks[index].name);
	...
	if (0 != failed) {
		...
		for (index = 0; index < failed_benchmarks_length; index++) {
			printf("%s[  FAILED  ]%s %s\n", colours[RED],
			       colours[RESET],
			       ubench_state.benchmarks[failed_benchmarks[index]]
				       .name);
		}
	}
